...
Vector2f MouseInfluenceRule::computeForce(const std::vector<Boid*>& neighborhood, Boid* boid) {
  ImGui::SetCurrentContext(world->engine->window->imGuiContext);
    ...
    if (ImGui::IsMousePosValid() && io.MouseDown[0])
    {
      ...
      Vector2f distanceFromMouse =  mousePos - boid->getPosition();
      const float distance = 500;

      if(distanceFromMouse.getMagnitude() > 0 && distanceFromMouse.getMagnitude() <= distance)
      {
        distanceFromMouse.normalized();
        distanceFromMouse /= distance;
        force = distanceFromMouse.normalized()/distanceFromMouse.getMagnitude();
      }
      if (isRepulsive)
      {
        force *= -1.0;
